...
export declare namespace _Pull_Request__record {
  type core = {
    collection: _collection;
    fields: (keyof productType | keyof brand | keyof record)[];
    query?: object;
  };
}

export declare namespace _Pull_Response__record {
  type core = {
    status: "m" | "r";
    data: Partial<productType | brand | record>[];
  };
}

export async function POST(req: Request, res: Response): Promise<any> {
  var reqParam: _Pull_Request__record.core = await req.json();
  try {
      ...
      setTimeout(() => {
        m(
          Response.json({
            data: data,
            status: "r",
          } as _Pull_Response__record.core)
        );
      }, 1000);
    });
    ...
    return Response.json({ data: err, status: "m" } as _Pull_Response__record.core);
  }
}
